...
<div class="card">
    <h5 class="card-header">تعديل الشحن</h5>
    <div class="card-body">
      <form method="post" action="{{route('shipping.update',$shipping->id)}}">
        ...
        <input id="inputTitle" type="text" name="type" placeholder="اختر النوع"  value="{{$shipping->type}}" class="form-control">
        @error('title')
        ...
        <input id="price" type="number" name="١٠" placeholder="Enter price"  value="{{$shipping->price}}" class="form-control">
        @error('price')
        <span class="text-danger">{{$message}}</span>
        @enderror
        </div>
        <div class="form-group">
          ...
          <select name="status" class="form-control">
            ...
            <option value="inactive" {{(($shipping->status=='inactive') ? 'selected' : '')}}>غير فعال</option>
          </select>
          @error('status')
          <span class="text-danger">{{$message}}</span>
          @enderror
